...
- If you installed CUDA in a custom versioned folder (for example `C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v13.2`), set `CUDA_PATH` (or `CUDAToolkit_ROOT`) to that root and rerun `scripts\update-addon-config.bat`. The script now prefers portable `$(CUDA_PATH)` and `$(VULKAN_SDK)` linker entries when those environment variables are available.
