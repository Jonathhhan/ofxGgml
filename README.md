...
- `ofxGgmlWorkflowManifest` provides the completed Phase 2 handoff contract for workflow inputs, typed stage contracts, artifacts, intermediate outputs, warnings, review notes, structured execution-step references, replay metadata, and downstream companion-tool routing
...
- `ofxGgmlTrustEvaluationSuite` provides a small Phase 3 reliability schema for trust criteria, deterministic scenarios, required reports, approval-first safety, and workflow-manifest handoffs
