...
testing. Add `-StrictModel` to fail fast when a non-embedding model is selected.

`run-text-example` and `run-chat-example` also support `-StrictModel` to fail fast
when a non-text GGUF is selected for chat/text requests.
