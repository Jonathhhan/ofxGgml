...
# local llama.cpp runtime optional. Model download is optional and disabled by default.
#
...
#   --with-llama-cli   Build optional local llama.cpp runtime (server + CLI fallback)
#   --skip-llama       Skip building llama.cpp runtime tools (legacy no-op)
...
if [[ "$SKIP_LLAMA" -eq 0 ]]; then
	write_step "Step 2/3: Building optional local llama.cpp runtime..."
	LLAMA_ARGS=()
	...
	write_ok "Optional llama.cpp runtime built successfully."
else
	write_warn "Skipping optional local llama.cpp runtime (server-first default)."
fi
...
echo "    4. Use --with-llama-cli if you want the local llama.cpp runtime"
echo "    5. Use scripts/start-llama-server.sh when you want a local persistent server"
echo ""
echo "  Examples:"
