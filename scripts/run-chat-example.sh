  ...
  case "$SERVER_URL" in
    http://127.0.0.1:*|http://localhost:*)
      if [ -n "$MODEL" ]; then
        ...
        sh "$SCRIPT_DIR/start-llama-server.sh" --server "$SERVER" --model "$MODEL" --port "$PORT" --detached
        READY=1
      else
...
if [ "$READY" -eq 0 ]; then
  echo "Launching anyway; the example will report endpoint status itself." >&2
fi
